...
function getPositiveArrayValues(arr) {
  ...
  const valuesArr = Array.from(iteratorValues);
  const positiveArrayValues = valuesArr.filter((value) => value > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArr = flatArr.map((element) => element ** 2);
  const filteredSquaredArray = squaredArr.filter(
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = slicedArr.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue,
    0
  );
...
function wordLengths(arr) {
  ...
  const lengthsArray = arr.map((words) => words.length);
  return lengthsArray;
}
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((currentLongest, word) => {
